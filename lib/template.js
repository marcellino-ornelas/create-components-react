...
Template.prototype._retrievePackage = function(package) {
  ...
  const compilePackageDirectory = function(srcPath, destPath) {
    ...
    const dirCompiledFiles = self._compileTemplateFiles(
      srcPath,
      destPath,
      files
    );

    const AllCompiledFiles = template.files.concat(dirCompiledFiles);

    template.files = AllCompiledFiles;
  };
...
Template.prototype._compileTemplateFiles = function(
  ...
  return allFiles.map(function(fileName) {
    return new File(srcPath, destPath, fileName);
  });
};
