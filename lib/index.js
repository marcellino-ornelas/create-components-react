...
function createReactComponents(dir, names) {
  ...
  var templates = new Template(templatePath, packages, TemplateOptions);
...
createReactComponents.initializeLocalSettings = function(cwd) {
  ...
  return fs
    ...
    .then(() =>
      ...
      // console.log('initialized settings in: ', SETTINGS_PATH)
      log('initialized settings in: ', SETTINGS_PATH)
    );
};
...
createReactComponents.initializeTemplates = function(cwd) {
  ...
  return fs.copy(DEFAULT_TEMPLATES_PATH, TEMPLATES_PATH).then(() =>
    // settings.get('verbose') &&
    // console.log('initializing templates at: ', TEMPLATES_PATH)
    log('initializing templates at: ', TEMPLATES_PATH)
  );
};
