...
Template.prototype.render = function(dest, data, cb) {
  ...
  fs.ensureDir(dest)
    ...
    .catch(err =>
      console.log(
        'Internal error: please report this error to as a github issue',
        err
      )
    );
};
...
Template.prototype._compileTemplateFiles = function(
  ...
  allFiles.forEach(function(fileName) {
    const file = new File(srcPath, destPath, fileName);
...
function File(src, dest, fileName) {
  ...
  const hasInterpolationInFileName = DOT_INTERPOLATION_MATCH.test(this.name);

  console.log('name', this.name);
  // console.log('name', this.name);
  console.log('file has dot interpolation:', hasInterpolationInFileName);

  if (hasInterpolationInFileName) {
    console.log('addign file Name');
    // compile dot interpolation in fileName if it has any
    this.fileName = (() => {
      const name = dot.template(this.name);
      return function(data) {
        return File.prototype.fileName.call({ name: name(data) });
      };
    })();
  }
