...
function Component( dir, name ){
  ...
  this.neededFolders = [];
	this.fileStructure = [
		...
		{ type: 'css', path: ( componentFilePath + settings.get('css') ) },
...
Component.prototype.createComponent = function(cb){
  ...
  fs.ensureDir( self.dir )
    .then(function(){
      console.log('Creating component ' + self.name + ' at ' + self.dir );
      ...
      async.each( self.fileStructure, function(fileData, done){
        self.createFile( fileData, function(err){
          ...
          if( err ){ 
            ...
            console.log('Please check out your component (path or name) to make sure there valid arguments');
            console.log('Error: ');
