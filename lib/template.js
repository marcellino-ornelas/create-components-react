...
Template.prototype.render = function(dest, data, cb) {
  ...
  fs.ensureDir(dest).then(function() {
    async.each(
      this.packages,
  ...
  return new Promise(function(resolve, reject) {
    async.each(
      ...
      function(file, done) {
        ...
        const data = Object.assign(dataForTemplates, { file: file });
        file = self._processPluginViews(file, data);
...
Template.prototype._processPluginViews = function(fileTemplate, data) {
...
Template.prototype._compileTemplateFiles = function(
  ...
  allFiles.forEach(function(fileName) {
    ...
    // check to see if there is more extentions in file name
    if (/\./g.test(fileInfo.name)) {
      const splitNameByExtentions = fileInfo.name.split('.');

      let extentions = splitNameByExtentions.slice(1).join('.');

      extentions += fileInfo.ext;

      fileInfo.name = splitNameByExtentions[0];
      // add dot back for file constructor correct parsing
      fileInfo.ext = extentions;
    }
...
function File(destPath, fileInfo, compiler) {
  ...
  this.needsToParseFileName = false;

  if (this._doesNameNeedToBeParsed()) {
    this.needsToParseFileName = true;
    this.templateName = dot.template(this.name);
  }

  let ext = fileInfo.ext || 'js';
...
File.from = function(_file) {
  return new File(_file.path, _file, _file.compiler);
};

File.prototype._doesNameNeedToBeParsed = function() {
  return /\{\{=([\s\S]+?)\}\}/g.test(this.name);
};

File.prototype.fileName = function(data) {
  return (
    (this.needsToParseFileName ? this.parseFileName(data) : this.name) +
    '.' +
    this.ext
  );
};

File.prototype.parseFileName = function(data) {
  return this.templateName(data);
};
