...
/*
  for every item in <names>
    set folder =  new Component from <item>
      
    make correct files paths from <folder>
      create files with correct contents inside
*/
  if( !names.length ){
    // default
    names.push("App");
  ...
  var fileNames = names.map(function( name ){
    return utils.makeFileNames( dir, name )
  });
  ...
  // console.log(dir.indexOf("src"));
  // if( dir.indexOf("src") === -1 ){
  //   console.log("No src folder detected. dont worry we will add one for you");
  //   // react website should have a src folder
  //   // if none is present it is added
  ...
  //   */
  //   let componentDir = parsedName.dir ? path.join(dir, parsedName.dir) : dir;

  //   // Capitalize first character of a word
  //   let capitalizedComponentName = utils.capitalize( parsedName.base );

  //   if( !capitalizedComponentName ) return;

  //   // make list of files that a component needs
  //   let filesNames = utils.makeFileNames( componentDir, capitalizedComponentName );

// 
  //   jsFiles.push([filesNames[0], capitalizedComponentName ])
